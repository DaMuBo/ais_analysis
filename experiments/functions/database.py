...
def create_backup(host,port, dbname, user, password, filename, pg_dump_path ='C:/Program Files/' ):
    """
    ...
    dumper = Path("./pg_dump")
    cmd = f'''cd "{str(pg_path)}" && {str(dumper)} --dbname={dbname} -h {host} -p {port} -U {user} -f {str(filename)}'''
    popen = Popen(cmd,stdin=PIPE,stderr=PIPE, universal_newlines=True, shell=True)
    return popen.communicate(f"{password}\n")

def load_backup(host,port, dbname, user, password, filename, pg_dump_path ='C:/Program Files/'):
    """
